...
def run_configuration(
    # add ieee like or hifloat config
    ...
    # IEEE like
    #module = build_fp_mul_sn(f"F{n_bits}Mul", EW=ew, FW=fw, subnormals=subnormals)
    
    # Hifloat
    # m = build_hif8_mul_logic("HiFP8Mul_Logic_Ref")

    # replace with fp multiplier test vectors
    ...
    for sigma_val, sw_val in zip(sigmas, switches):
        # create new row structure make new row
        row = MultiplierRow(
                    run_id=run_id,
                    timestamp=t_now,
